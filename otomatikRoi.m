...
hratio=(I1info(s2).Height+i)/I1info(s).Height;
wratio=(I1info(s2).Width+j)/I1info(s).Width;

roi(2:2:4)=roi(2:2:4)*hratio;
roi(1:2:3)=roi(1:2:3)*wratio;
image_thresholded=imclose(image_thresholded,B);
%%
...
%montage({segmented_images(:,:,:,1),segmented_images(:,:,:,2),segmented_images(:,:,:,3)});
